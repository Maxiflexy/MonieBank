    ...
    public RouteLocator routes(RouteLocatorBuilder builder) {
        return builder.routes()
                ...
                .route("auth-service-login", r -> r.path("/api/auth/login")
                        .uri("lb://auth-service"))
                .route("auth-service-refresh", r -> r.path("/api/auth/refresh")
                        .uri("lb://auth-service"))
                ...
                .route("auth-service-user-validation", r -> r.path("/api/auth/user/{userId}")
                        .uri("lb://auth-service"))
                .route("auth-service-validate-token", r -> r.path("/api/auth/validate-token")
                        .uri("lb://auth-service"))

                // Auth Service Routes (Secured)
                .route("auth-service-logout", r -> r.path("/api/auth/logout")
